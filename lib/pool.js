 ...
 */
 Pool.prototype.add = function(host, socket) {
  ...
  if (!socket instanceof netSocket || socket._handle.fd <= 0) 
    return false;
  // convert to a Pool socket
  var pSocket = new Socket(socket);
  pSocket.pid  = generateId(this.sockets[host.host + ':' + host.port]);
...
Pool.prototype._ensure = function() {
  ...
  if (this.minimum < available_len && socket_len < this.maximum) {
    ...
    sock.once('connect', function() {
      ...
      this.pid = generateId(self.sockets[server.host + ':' + server.port]);
      self.sockets[server.host + ':' + server.port][this.pid] = this;
      self._available(this);
    })
